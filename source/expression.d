...
struct Expression(V)
{
    ...
    this(R)(R source) if (isInputRange!R && isSomeChar!(ElementType!R))
    {
        ...
        m_root = compileTertiary(src, m_context);
        skipWS(src);
        if (!src.empty())
        {
            throw new ExpressionError(src, "syntax error, unexpected '" ~ src.front.to!string ~ "'");
        }
    }
    ...
    auto variables()
    {
        return m_context.variables.keys;
    }

    auto functions()
    {
        return m_context.functions.keys;
    }
...
private:
    ...
    void validate()
    {
        if (!m_validated)
        {
            ...
            {
                if (!var.isSet)
                    throw new ExpressionError("uninitialized variable: " ~ var.name);
            }
            foreach (fn; m_context.functions.values)
            {
                if (!fn.isSet)
                    throw new ExpressionError("uninitialized function: " ~ fn.name);
            }
            m_validated = true;
        }
    }
...
alias compileExpr = compileTertiary;

Node!V compileTertiary(R, V)(ref Source!R src, ref Context!V ctx)
{
    auto arg = compileComparison(src, ctx);
    skipWS(src);
    while (!src.empty)
    {
        switch (src.front)
        ...
        case '?':
            ...
            auto left = compileComparison(src, ctx);

            skipWS(src);
            if (src.front != ':')
            {
                throw new ExpressionError(src, "syntax error, incomplete operator '?:' detected");
            }
            src.popFront();

            arg = new Tertiary!(V, "?")(arg, left, compileComparison(src, ctx));
...
Node!V compileComparison(R, V)(ref Source!R src, ref Context!V ctx)
{
    auto arg = compileAdd(src, ctx);
    skipWS(src);
    while (!src.empty)
    {
        switch (src.front)
        ...
        case '=':
            ...
            {
                src.popFront();
                arg = new Binary!(V, "==")(arg, compileAdd(src, ctx));
            }
            else
            {
                throw new ExpressionError(src, "syntax error, incomplete operator '==' detected");
...
Node!V compileValue(R, V)(ref Source!R src, ref Context!V ctx)
{
    ...
    if (isAlpha(src.front))
    {
        ...
        if (!src.empty && src.front == '(')
        {
            ...
            while (!src.empty)
            {
                ...
                {
                    if (src.front != ',')
                        throw new ExpressionError(src, "comma expected");
                    src.popFront();
                }
    ...
    if (src.front == '(')
    {
        ...
        skipWS(src);
        if (src.front != ')')
            throw new ExpressionError(src, "closing parenthesis expected");
        src.popFront();
        return expr;
    }
...
{
    while (!src.empty && isWhite(src.front))
        src.popFront();
}
...
class Literal(V) : Node!V
{
    ...
    {
        return "Literal(" ~ m_val.to!string ~ ")";
    }
}
...
class Tertiary(V, string op) : Node!V
{
    protected Node[3] m_args;
    this(Node cond, Node success, Node fail)
    {
        m_args = [cond, success, fail];
    }

    V opCall() const
    {
        V test = mixin(`m_args[0]()`);

        if (test > 0)
        {
            return mixin(`m_args[1]()`);
        }
        else
        {
            return mixin(`m_args[2]()`);
        }
    }
...
class Function(V) : Node!V
{
    ...
    override string toString() const
    {
        return format!"Function(%s)"(arity);
    }
}
...
unittest
{
    ...
    assert(expr("dist ( sqrt ( a ) , b ) * ( a + 5.8 ) / 3 + b / a - ( 235.6 + 3 * b) / - 2.5 + dist ( a )")
            .isClose(result));
}
...
unittest
{
    ...
    assert(expr("a < b ? a : b") == 3);
    assert(expr("a <= b ? b : a") == 9);
    assert(expr("a != b ? 0 : 1") == 0);
    assert(expr("b > a ? 2 : 1") == 2);
}
